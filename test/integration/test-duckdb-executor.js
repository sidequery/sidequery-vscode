...
await test('Query type detection', async () => {
    ...
    for (const query of selectQueries) {
        ...
        assert(result.arrowPath !== undefined || result.error !== undefined, `Query "${query}" should attempt Arrow export or have error`);
    }
    ...
    for (const query of nonSelectQueries) {
        const result = await executor.executeQuery(query, ':memory:');
        assert(result.arrowPath === undefined, `Query "${query}" should not produce Arrow file`);
    }
});
// Summary
console.log(`\n📊 Results: ${passed} passed, ${failed} failed, ${passed + failed} total`);
process.exit(failed > 0 ? 1 : 0);
//# sourceMappingURL=test-duckdb-executor.js.map